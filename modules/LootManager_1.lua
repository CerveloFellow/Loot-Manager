...
function LootManager.new(config, utils, itemEvaluator, corpseManager, navigation, actorManager, itemScore)
    ...
    function self.lootAllQueuedItemsFromCorpse(corpseId, queuedItems)
        ...
        -- Build a lookup of what we need to loot: itemId -> {queuedItem, remaining count, looted count}
        local itemsToLoot = {}
        for _, queuedItem in ipairs(queuedItems) do
            itemsToLoot[queuedItem.itemId] = {
        ...
        -- Single pass through corpse items, looting everything we need
        for i = 1, corpseItemCount do
            ...
            -- Check if this item is in our loot list and we still need more
            local lootEntry = itemsToLoot[localItemId]
            if lootEntry and lootEntry.remaining > 0 then
                self.debugPrint(string.format("Found queued item match: %s in slot %d (need %d more)", 
                    corpseItem.Name(), i, lootEntry.remaining))
                ...
                self.debugPrint(string.format("Looted %s (%d/%d)", 
                    corpseItem.Name(), lootEntry.looted, lootEntry.queuedItem.count or 1))
            end
        ...
        for itemId, lootEntry in pairs(itemsToLoot) do
